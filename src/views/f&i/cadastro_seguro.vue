<template>
      ...
      <div class="row g-2 p-2">
        ...
        <div class="card-title gy-4">
          <i class="bi bi-journal-text fs-5 icone_filtro"
            ><span class="texto_filtro">Dados do Seguro</span></i
          >
        </div>
      </div>
      <div class="row g-2 p-2">
  ...
  <div class="card card-tabela g-2 p-2 rf_margin">
    <table class="table rf_texto">
      ...
      <tbody>
        <tr v-for="item in seguros" :key="item.id" class="table-linha">
          <td>{{ item.codigo }}</td>
          <td>{{ item.descricao }}</td>
          <td>{{ item.seguros_modelo.descricao }}</td>
  ...
  methods: {
    ...
    async retrieveSeguros() {
      ...
      try {
        ...
        this.seguros = response.data.seguros; // exemplo de atribuição
    ...
    async update_seguros() {
      ...
      try {
        ...
        await axios
          .put(`${process.env.VUE_APP_API_URL}listar_seguros/${id}`, dados, {
            headers: {
    ...
    async onSubmit() {
      await fetch(`${process.env.VUE_APP_API_URL}seguros`, {
        method: "POST",
