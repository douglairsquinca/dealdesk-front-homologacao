<template>
  ...
  <NavgatorDk
    ref="navgator"
    :barraTitulo="' Fluxo de Lojas - F&I'"
    :titulo="'relatório'"
  />
    ...
    <div class="row g-2 p-2">
      <div class="card-title rf_texto gy-4">
        <i class="bi bi-funnel fs-5 icone_filtro"
          ><span class="texto_filtro">Parâmetros do Relatório</span></i
        >
      </div>
      <div class="row p-2">
        <div class="col-2 col-md-4">
          <div class="form-floating">
            ...
            >
              <option value="menu_gerados">Menus Gerados</option>
              <option value="vendas">Vendas</option>
              <option value="venda_perdida">Vendas Perdidas</option>
              <option value="venda_conversao">Conversao de Vendas</option>
            </select>
            <label class="rf_texto"
              ><strong>Escolha o tipo de Relatório</strong></label
            >
          </div>
        </div>
        <div class="col-2 col-md-6">
          ...
          >
            ...
            <div class="d-flex justify-content-between">
              <div class="">
                <div
                  class="form-check form-check-inline rf_texto_desk"
                  v-for="item in tipo_conversao"
                  :key="item.value"
                >
                  <input
  ...
  <div class="card card-tabela g-2 p-2">
      ...
      <div class="col-12">
        <div class="card-title rf_texto gy-4">
          <i class="bi bi-shop fs-5 icone_filtro"
            ><span class="texto_filtro">Lojas</span></i
          >
        </div>
        <div class="">
      ...
      <div class="col-1 col-md-3">
        <div class="form-floating">
          <button
            ...
            @click="
              selectedRelatorio === 'venda_conversao'
                ? relatorio_conversao_vendas()
                : selectedRelatorio === 'venda_perdida'
                ? relatorio_venda_perdida()
                : selectedRelatorio === 'vendas'
                ? relatorio_vendas()
                : selectedRelatorio === 'menu_gerados'
                ? relatorio_menu_gerados()
                : null
            "
          >
    ...
    <div class="row g-2 p-2">
      <div class="card-title rf_texto gy-4">
        <i class="bi bi-printer fs-5 icone_filtro"
          ><span class="texto_filtro">Imprimir Relatórios</span></i
        >
      </div>
      <div class="col-1 col-md-2">
        <button
          class="btn btn-lg btn-filtro"
          v-if="selectedRelatorio === 'venda_conversao'"
          @click="exportToXLSX"
        >
    ...
    <table
      class="table border-dark rf_texto mt-2"
      v-if="selectedRelatorio === 'venda_perdida'"
      id="VendaPerdida"
    >
      <thead>
        <tr>
          <th scope="col" class="rf_header_table">
            <button
              class="btn btn-secondary mt-2 rf_btn"
              v-if="selectedRelatorio === 'venda_perdida'"
          ...
          <th scope="col" class="rf_header_table">
            <button
              ...
              v-if="selectedRelatorio === 'venda_perdida'"
              data-bs-toggle="modal"
              data-bs-target="#ModalVendaPerdida"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="16"
      ...
      <tbody>
        <tr v-for="item in vendas_perdidas" :key="item.id">
          ...
          <td class="rf_header_table">
            {{ formattedHoras(item.inicio_atendimento) }}
          </td>
    ...
    <table
      id="tabela_venda_modelo"
      class="table border-dark rf_texto mt-2"
      v-if="selectedRelatorio === 'vendas'"
    >
      <thead>
        <tr>
          <th scope="col" class="rf_header_table">
            ...
            <button
              ...
              v-if="selectedRelatorio === 'vendas'"
              data-bs-toggle="modal"
              data-bs-target="#ModalVendaModelo"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="16"
                      ...
                      >
                        ...
                        <tbody>
                          <tr v-for="item in conversao_vendas" :key="item.id">
                            ...
                            <td class="rf_header_table">
                              {{ item.QuantidadeVendasUsados }}
                            </td>
                            <td class="rf_header_table">
                              {{ item.ConversaoUsados }}
                            </td>
                            <!--Bloco todos-->
                            <td class="rf_header_table">
                      ...
                      >
                        <thead>
                          <tr>
                            ...
                            <th
                              scope="col"
                              class="rf_header_table"
                              v-if="filtroSelecionadoLabel !== 'Loja'"
                            >
                              Valor
                            </th>
                            <th
...
export default {
  ...
  methods: {
    async relatorio_vendas(){
        console.log("Chhamar relátorio de vendas");
      ...
      try {
        ...
        console.log("Relatorio de Atendimentos", response.data.result)
        for (let i = 0; i < qtd; i++) {
          ...
          const arr = {
            loja: response.data.result[i].Loja,           
            vendedor: response.data.result[i].Descricao,
    ...
    getRequestParams(EmpresasSelecionadas, page, pageSize, empresa_id, usuario_id, data_ini, data_fim, tipo_conversao, referencia) {
      let params = {};
    ...
    selecionarRelatorios() {
      if (this.selectedRelatorio === "venda_conversao") {
        ...
        this.tipo_relatorio = this.selectedRelatorio
      } else if (this.selectedRelatorio === "venda") {
        this.tipo_relatorio = this.selectedRelatorio;
        this.tipo_conversao = [
          { nome: 'Loja', value: 1 },
          { nome: 'Vendedor', value: 2 }
        ]
      } else if (this.selectedRelatorio === "menus_gerados") {
        this.tipo_relatorio = this.selectedRelatorio;        
      }
