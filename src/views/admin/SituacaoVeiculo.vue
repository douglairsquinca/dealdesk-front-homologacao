  ...
  <div>
    ...
    <nav class="navbar navbar-expand-lg rf_bg_form rf_texto rf_container">
      <div class="container-fluid">
        <div><i class="bi  bi-file-earmark-text fs-5"> Verificar situação do veículo </i></div>
      </div>
    </nav>
...
export default {
  ...
  methods: {
    ...
    async retrieveModelos() {
      try {
        if (!this.searchPlaca && !this.searchChassi) {
          // Se nenhum dos campos foi fornecido, chame o método original
          await this.retrievePropostas();
          return;
        }


        const response = await axios.get(`${process.env.VUE_APP_API_URL}veiculo_vendido_estoque`);
        const veiculosFiltrados = [];

        for (const dados of response.data.veiculoEstoque) {
          if ((this.searchPlaca && dados["proposta_veiculo"]["placa"].includes(this.searchPlaca)) ||
            (this.searchChassi && dados["proposta_veiculo"]["chassis"].includes(this.searchChassi))) {
            ...
            veiculosFiltrados.push(veiculo);
          }
        }
