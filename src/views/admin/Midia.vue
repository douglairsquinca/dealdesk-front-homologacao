      ...
      <div class="container-fluid">
        ...
        <div>
          <ul class="nav justify-content-end">
            ...
            <li class="nav-item">
              <a class="nav-link rf_texto disabled">Mídia</a>
            </li>
          </ul>
        </div>
      </div>
  ...
  methods: {
    ...
    async onSubmit() {
      try {
        ...
        const resp = await fetch(`${process.env.VUE_APP_API_URL}midia`, {
          method: "POST",
    ...
    async retrieveMidias() {
      try {
        ...
        userService.getMidias(params).then((response) => {
          const { midias, totalPages, totalItems } = response.data;
          this.midias = midias;
    ...
    async update() {
      ...
      const response = await axios.put(
        `${process.env.VUE_APP_API_URL}midia/${this.edit_id}`,
        dados,
        { headers }
      );
